        ...
        <div class="row page-titles">
            ...
            <div class="col-md-7 align-self-center text-end">
                <div class="d-flex justify-content-end align-items-center">
                    ...
                    <button type="button" class="btn btn-info d-none d-lg-block m-l-15 text-white"><i class="fa fa-plus-circle"></i> {{isset($frequency) && ($frequency->pk_frequency) ? 'Edit Frequency' : 'Create New Frequency'}}</button>
                </div>
            </div>
        </div>
        <div class="row">
                                ...
                                <div class="tab-content br-n pn">
                                    <div id="navpills-1" class="tab-pane active">
                                        <div class="row" style="margin-left:580px;">
                                          ...
                                          @endif
                                          <form class="form-horizontal mt-4 " method="post" action="{{isset($frequency) && ($frequency->pk_frequency) ? '/superadmin/frequency/update' : '/superadmin/frequency/submit'}}">
                                            @csrf
                                            <div class="form-group">
                                                <label for="country_code">Frequency</label>
                                                <input type="text" name="frequency" class="form-control @error('frequency') is-invalid @enderror" value="{{isset($frequency) && ($frequency->frequency)?$frequency->frequency:''}}">
                                                @error('frequency')
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="description" class="form-control @error('description') is-invalid @enderror" value="{{isset($frequency) && ($frequency->description)?$frequency->description:''}}">
                                                @error('description')
                                                    <span class="invalid-feedback" role="alert">
                                                        <strong>{{ $message }}</strong>
                                                    </span>
                                            ...
                                            <input class="btn btn-primary" type="submit" value="{{isset($frequency) && ($frequency->pk_frequency)?'Update':'Submit'}}">
                                          </form>
                                        </div>
                                    </div>
                                </div>
