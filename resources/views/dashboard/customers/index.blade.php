                            ...
                            <div class="table-responsive m-t-40">
                                <table id="example23"
                                    ...
                                    @foreach($orders as $value)
                                        <tr>
                                            <td onclick="window.location='{{ route('dashboard.myorderdetails', ['id' => $value->pk_orders]) }}'">
                                                #{!! $value->pk_orders !!}</td>
                                            <td onclick="window.location='{{ route('dashboard.myorderdetails', ['id' => $value->pk_orders]) }}'"
                                                class="text-center">{!! Helper::formatDate($value->created_at) !!}</td>
                                            <td onclick="window.location='{{ route('dashboard.myorderdetails', ['id' => $value->pk_orders]) }}'">{!! ucfirst($value->order_status) !!}</td>
                                            <td onclick="window.location='{{ route('dashboard.myorderdetails', ['id' => $value->pk_orders]) }}'">
                                                ${!! number_format($value->total,2) !!}</td>
                                            <td style="text-align:center;">
                                                <a href="{!! route('dashboard.myorderdetails',array($value->pk_orders)) !!}">
                                                    <button class="btn btn-success text-white">View</button>
                                                </a>
                                                @if(!in_array($value->pk_order_status, [3, 5, 6]))
                                                    <a href="/order/cancel-order/{{$value->pk_orders}}"
